...
## CV bullets (updated to match measured reality)

```
Architected an autonomous luggage-transport robot in PyBullet using
pure-pursuit path tracking and reactive A* collision detours on a
19-segment airport tarmac, achieving 100% navigation success over 100
randomized-traffic evaluation episodes (mean 25.6s depot-to-gate).

Implemented tabular Q-learning for gap acceptance at road crossings
with a direction-aware state space and outcome-based delayed rewards,
persisting the Q-table to JSON so go/wait decisions measurably improve
across episodes.

Designed predictive crossing safety that estimates each vehicle's
time-to-crossing from position, speed, and direction — replacing
distance thresholds that were geometrically unsatisfiable — and a
collision-recovery state machine (reverse, replan, rejoin) covering
vehicles and structures.

Built an automated evaluation harness that runs headless simulation
episodes in isolated subprocesses and produces success-rate, timing,
and Q-convergence learning curves, turning an estimated success rate
into a measured one.
```
